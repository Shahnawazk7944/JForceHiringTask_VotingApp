...
import com.example.jforcehiringtask.components.FThirdPartyAuthButton
...
fun SignUp(
    ...
    StoreSignUpContent(navController = navController)
}
    ...
    Scaffold(
        ...
        Column(
            ...
            OutlinedTextField(
                value = contactNumber,
                onValueChange = {
                    if (it.isDigitsOnly()) { contactNumber = it}
                },
                textStyle = TextStyle(
                    fontFamily = poppins,
                    fontSize = 20.sp,
                ...
                onClick = {
                    if (email.isNotEmpty() && password.isNotEmpty() && name.isNotEmpty()) {
                        when (true) {
                            ...
                            (contactNumber.length < 10) -> {
                                scope.launch {
                                    snackbarState.currentSnackbarData?.dismiss()
                                    snackbarState.showSnackbar(
                                        message = "Short Number! Enter at least 10 characters.",
                                        actionLabel = "Retry",
                                        duration = SnackbarDuration.Short
                                    )
                                }
                        ...
                        scope.launch {
                            snackbarState.currentSnackbarData?.dismiss()
                            snackbarState.showSnackbar(
                                message = "Name, Number, Email, Password are mandatory.",
                                actionLabel = "Retry",
                                duration = SnackbarDuration.Short
                            )
                        }
